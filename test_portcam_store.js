...
test('panel and Focus map state is UI-only, clear focus preserves Active and Current selections, and undo keeps it', () => {
  const s = store(); s.getObservation('a', 't1'); s.markSaved(); s.setFocusedEntity('camera', 'b');
  const before = s.getState(); s.setPanelOpen('objectManager', false); s.setPanelOpen('inspector', false); s.setFocusMapMode(true); s.clearFocusedEntity();
  const focusedCleared = s.getState(); assert.equal(focusedCleared.uiState.focusMapMode, true); assert.equal(focusedCleared.uiState.focusedEntity, null); assert.equal(focusedCleared.uiState.selectedCameraId, 'b'); assert.equal(focusedCleared.uiState.selectedTargetId, 't1'); assert.deepEqual(focusedCleared.history, before.history); assert.equal(focusedCleared.dirty, false); assert.equal(JSON.stringify(s.toCameraProject()).includes('focusMapMode'), false);
  s.beginPreview('camera', 'b', {headingDeg: 361.25}); assert.equal(s.getCamera('b').headingDeg, 1.25); s.cancelPreview(); s.beginPreview('camera', 'b', {headingDeg: -1}); s.commitPreview('heading-scrub');
  assert.equal(s.getCamera('b').headingDeg, 359); assert.equal(s.getState().history.length, 1); s.undo(); const undone = s.getState(); assert.equal(undone.uiState.focusMapMode, true); assert.equal(undone.uiState.focusedEntity, null); assert.equal(undone.uiState.selectedCameraId, 'b'); assert.equal(undone.uiState.selectedTargetId, 't1'); s.redo(); assert.equal(s.getCamera('b').headingDeg, 359);
});
